...
They are configured in a way to allow seamless working using Arch Linux in WSL2 on Windows.
...
the packages `WSL` and `win32yank` are necessary. The rest is just other
programs I use.

### Install Arch Linux for WSL
To install Arch Linux follow [this](https://wiki.archlinux.org/title/Install_Arch_Linux_on_WSL)
...
with admin privileges in PowerShell to allow inbound connections:
```
...
### Git
To be able to obtain this repository with all it's files it is recommended
...
Note that the location is important as paths that reference the submodules point
here to avoid having to symlink those as well.
...
> [!NOTE]
> It is recommended to set `recurse.submodules` locally so that the submodule
> commits are always synced to those in the repository for consistent behavior
> and to allow for explicit upgrading.
>
> ```bash
> git config submodule.recurse true
> ```

Before you can use the config the packages to be configured need to be installed.

### Install ZSH
The next step is to install ZSH as most of the config is built on top of it.
To do that just run the provided script with system user privileges
```
...
```
To apply the configuration everything can be symlinked to the correct place by running:
```
